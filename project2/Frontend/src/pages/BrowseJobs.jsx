...
const locations = ["all", ...new Set((Array.isArray(jobs) ? jobs : []).map(job => job.location).filter(Boolean))];

const types = ["all", ...new Set((Array.isArray(jobs) ? jobs : []).map(job => job.type).filter(Boolean))];
